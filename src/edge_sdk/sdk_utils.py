...
def update_inference_data_path(data, new_path):
    for item in data['external_path']:
...
def extract_selected_user_parameters(data):
    ...
    for item in data["pipeline"]:
        if item["type"] == "train":
            train_selected_user_parameters = item["parameters"]["selected_user_parameters"]
        elif item["type"] == "inference":
            inference_selected_user_parameters = item["parameters"]["selected_user_parameters"]
